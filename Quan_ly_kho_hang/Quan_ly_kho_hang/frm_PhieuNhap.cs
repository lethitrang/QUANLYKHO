    ...
    public partial class frm_PhieuNhap : Form
    {
      ...
      //          ec_pn.TongTien = int.Parse(txtTongTien.Text);
        ...
        private void btnLamMoi_Click(object sender, EventArgs e)// Phiếu nhập
        {
            ...
            SetNull_PN();
            SetNull_CTPN();
            dgvCTPhieuNhap.DataSource = sql_ctpn.getChitiet();
...
//                btnXemCTPN.Enabled = true;
            ...
            else                         // đang ở trạng thái thêm phiếu nhập
            {
                ...
                MessageBox.Show("Bạn đã thêm vào chi tiết phiếu nhập thành cônng", "Thông Báo", MessageBoxButtons.OK);
                dgvCTPhieuNhap.DataSource = bus_ctpn.getChitiet_MaPN(txtMaPH.Text);
